<?php if (count($cart) > 0): ?>
	...
	<tr scope="row" class="border-0">
		...
		<td class="border-0 align-middle"><a href="#" class="text-dark class remove-item" data-name="<?= $product['name'] ?>" data-stock_id="<?= $product['stock_id'] ?>"><i class="fa fa-trash"></i></a></strong></td>
	</tr>
	<?php endforeach ?>
<?php else: ?>
	<tr scope="row" class="border-0">
