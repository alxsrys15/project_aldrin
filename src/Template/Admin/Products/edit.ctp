...
<div class="row">
    ...
    <div class="col-md-6">
        ...
        <div class="row">
            <div class="col-4">
                <?= $this->Html->image('product_images/'.$product['images'][0], ['class' => 'mx-auto img-fluid preview', 'data-input' => '#img1', 'style' => ['cursor: pointer'],'id' => 'image1-preview']) ?>
            ...
            <div class="col-4">
                <?= $this->Html->image('product_images/'.$product['images'][1], ['class' => 'mx-auto img-fluid preview', 'data-input' => '#img2', 'style' => ['cursor: pointer'],'id' => 'image2-preview']) ?>
                <div class="custom-file d-none">
            ...
            <div class="col-4">
                <?= $this->Html->image('product_images/'.$product['images'][2], ['class' => 'mx-auto img-fluid preview', 'data-input' => '#img3', 'style' => ['cursor: pointer'],'id' => 'image3-preview']) ?>
                <div class="custom-file d-none">
        ...
        <?php foreach ($product->product_stocks as $key => $stock): ?>
        <div class="form-row align-items-center mb-3">
            <?= $this->Form->input('product_stocks.'.$key.'.id', ['type' => 'hidden', 'value' => $stock->id]) ?>
            <div class="col-4">
                <label class="sr-only">Size</label>
                <?= $this->Form->input('product_stocks.'.$key.'.size_id', ['class' => 'form-control', 'type' => 'select', 'label' => false, 'value' => $stock->size_id]) ?>
            </div>
            <div class="col-4">
                <label class="sr-only">Stock</label>
                <?= $this->Form->input('product_stocks.'.$key.'.variant', ['class' => 'form-control', 'type' => 'text', 'label' => false, 'placeholder' => 'Variant/Color', 'required']) ?>
            </div>
            <div class="col-3">
                <label class="sr-only">Stock</label>
                <?= $this->Form->input('product_stocks.'.$key.'.sku', ['class' => 'form-control sku', 'type' => 'number', 'min' => 1, 'label' => false, 'placeholder' => 'Quantity', 'required']) ?>
            </div>
            <div class="col-1">
...
<?= $this->Form->button('Update Product', ['type' => 'submit', 'class' => 'btn btn-dark']) ?>
<?= $this->Form->end() ?>
